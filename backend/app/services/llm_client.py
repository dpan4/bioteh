...
SYSTEM_PROMPT_TEMPLATE = (
    ...
    "   ❌ НЕПРАВИЛЬНО: Нельзя напрямую установить startDate или endDate!\n"
    "   ✅ ПРАВИЛЬНО: Управляй датами через duration_days И predecessors:\n\n"
    
    "   Алгоритм для установки конкретных дат:\n"
    "   a) Вычисли duration_days = (endDate - startDate + 1)\n"
    "   b) Определи predecessors для сдвига даты начала:\n"
    "      - Если задача должна начаться РАНЬШЕ, чем сейчас → УДАЛИ predecessors (= [])\n"
    "      - Если задача должна начаться ПОЗЖЕ → найди задачу(и), которая(ые) заканчивается(ются)\n"
    "        незадолго до нужной даты начала, и добавь её(их) в predecessors\n"
    "      - Если predecessors оставить как есть, даты пересчитаются по старому графу!\n\n"
    
    "   Пример 1: «task-2 должна быть со 2 по 18 августа» (сейчас: с 6 по 10 августа с predecessor [\"task-1\"]):\n"
    "     - duration_days = 18 - 2 + 1 = 17\n"
    "     - Начало РАНЬШЕ текущего (2 < 6) → УДАЛИ predecessors = []\n"
    "     - Вызов: update_task_details(task_id='task-2', duration_days=17, predecessors=[])\n\n"
    
    "   Пример 2: «task-6 должна быть с 20 августа по 2 сентября после task-5»:\n"
    "     - duration_days = (31 - 20 + 1) + 2 = 14\n"
    "     - task-5 заканчивается 21 августа, нужно начать 20 августа\n"
    "     - НЕ добавляй task-5 в predecessors, т.к. нужно начать ДО её окончания!\n"
    "     - Найди задачу, которая заканчивается около 19 августа, или оставь predecessors = []\n"
    "     - Если ОБЯЗАТЕЛЬНО после task-5 → predecessors = ['task-5'], но тогда начнётся ПОСЛЕ окончания task-5!\n\n"

    "5. При добавлении/изменении задач с конкретными датами — точно вычисляй "
    "duration_days по формуле выше и ВНИМАТЕЛЬНО выставляй predecessors по алгоритму из пункта 4!\n\n"
